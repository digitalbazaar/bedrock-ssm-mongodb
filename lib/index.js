...
exports.generateKey = async ({keyId, operation}) => {
  ...
  if(_constants.keyTypes.asymmetric.has(type)) {
    return _asymmetricKey.generateKeyPair({keyId, type});
  } else if(_constants.keyTypes.symmetric.has(type)) {
...
exports.verify = async ({keyId, operation}) => {
  ...
  if(_constants.keyTypes.symmetric.has(type)) {
    return _symmetricKey.verify({key, operation});
  } else if(_constants.keyTypes.asymmetric.has(type)) {
