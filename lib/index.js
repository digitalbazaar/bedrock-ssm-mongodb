...
const {util: {BedrockError, uuid}} = bedrock;
 ...
 */
exports.generateKey = async ({type}) => {
 ...
 */
exports.wrapKey = async ({kekId, key}) => {
  assert.string(kekId, 'options.kekId');
  assert.string(key, 'options.key');

  const {key: kek} = await _getKeyRecord({id: kekId});
 ...
 */
exports.unwrapKey = async ({kekId, wrappedKey}) => {
  assert.string(kekId, 'options.kekId');
 ...
 */
exports.verify = async ({keyId, data, signature}) => {
  assert.string(keyId, 'options.keyId');
  assert.string(data, 'options.data');
