...
const {
  CONTEXT_URL: WEBKMS_CONTEXT_URL
} = require('@digitalbazaar/webkms-context');
 ...
 */
exports.generateKey = async ({keyId, type, controller} = {}) => {
  const keyContextUrl = _constants.KEY_TYPES.symmetric.get(type);
  if(!keyContextUrl) {
  ...
  try {
    ...
    const keyDescription = {
      '@context': [
        WEBKMS_CONTEXT_URL,
        _constants.KEY_TYPES.symmetric.get(type)
      ],
      id: keyId,
      type,
      controller
