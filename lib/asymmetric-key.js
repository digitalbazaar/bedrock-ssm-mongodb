...
import * as ecdsa from './ecdsa/index.js';
...
export async function generateKeyPair({
  ...
  if(keyDescriptionType === 'Multikey') {
    keyDescription['@context'] = 'https://w3id.org/security/suites/multikey/v1';
...
export async function sign({key, operation}) {
  ...
  if(key.type.startsWith('urn:webkms:multikey:P-')) {
    const keyPair = await ecdsa.from(key);
    // verify data must be prehashed before sending to WebKMS
    signer = keyPair.signer({hashAlgorithm: null});
  } else {
