...
export async function generateKeyPair({curve} = {}) {
  ...
  return {
    ...
    // FIXME: it could also be the case that signer() always takes a hash
    // algorithm in a separate ecdsa lib ... and signer() for WebKMS
    // AsymmetricKey will require one for ECDSA keys ... and just do the
    // hashing locally? ... if so, that lib wouldn't be usable here and
    // we'd need to provide our own internal implementation
